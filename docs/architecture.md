...
35 tools from a lofted solid in about a second, and the whole 22-configuration
matrix builds in ~37 s of kernel time. So the through/partial distinction stops
...
| Suite | What it pins |
| --- | --- |
| `wheel.test.js` (76) | Chunking math, joint clearance, dedupe, bolt/seam clearance, bore schemes, and the web patterns' guarantees — cell-to-cell wall, no overlap, no nesting, no self-crossing loops, band containment, seam clearance, measured on the finished millimetre geometry across 31 configurations. Also the material-zone invariant: 2 100 configurations, nothing the planner cuts crosses a zone boundary, so the two bodies always meet on a complete annulus. |
| `occgen.test.js` (43) | The emitted bundle across a 16-config matrix. The data blocks are read back and checked as geometry rather than as text: every loop closes to 1e-6 mm, every arc agrees with its own centre, cutters carry a sane depth range, a crowned piece emits congruent sections, the emitted `ZONES` tile the piece with no gap, and a configuration regenerates byte-identically. |
...
Above that, `npm run validate` regenerates a 22-configuration matrix and — when
...
The whole matrix currently builds **22/22, 33 pieces, ~38 s of kernel time**,
...
the planner asked for — see §12. For contrast, the 19 configurations it started
as, against the hosted
